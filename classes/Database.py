...
class Database(object):
        ...
        def get_tree(self):
            ...
            for row_channel in data_channels:
                ...
                for row_group in data_groups:
                    tree_list.append({'name': row_group.get('dali_group_name'), 'type': 2, 'id_of_type': row_group.get('ID')})
                    sql_string = 'SELECT * FROM ballasts WHERE ballast_group_' + str(row_group.get('dali_group_number')) + '=1 ORDER BY ballast_short_address ASC'
                    c.execute(sql_string)
                ...
                sql_string = 'SELECT * FROM ballasts WHERE ballast_group_0=0 AND ' + \
                             ...
                             ' ballast_group_1=0  AND ballast_group_2=0 AND ballast_group_3=0 AND ' + \
                             ' ballast_group_1=4  AND ballast_group_5=0 AND ballast_group_6=0 AND ' + \
                             ...
                             ' ballast_group_13=0  AND ballast_group_14=0 AND ballast_group_15=0'
                c.execute(sql_string)
                data_ballasts = c.fetchall()
                for row_ballast in data_ballasts:
                    tree_list.append(
                        {'name': row_ballast.get('ballast_name'), 'type': 3, 'id_of_type': row_ballast.get('ID')})
        ...
        def insert_visual(self, visual_name, visual_page_ID, visual_type, visual_columns, visual_ID_of_type):
            ...
            c = self.conn.cursor()
            sql_string = 'SELECT visual_sort_order FROM visual WHERE visual_page_ID= ' + str(visual_page_ID) + ' ORDER BY visual_sort_order DESC LIMIT 1'
            c.execute(sql_string)
            data = c.fetchone()
            sortOrder = data[0]
